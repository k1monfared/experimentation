...
There are also questions the horizon itself rules out. A career-outcomes study or a public-health intervention whose effect only shows up after five years cannot be randomized cleanly: users leave, the world around them changes, the control arm contaminates the treatment arm, and by the time the results arrive the thing being measured is no longer the thing that was assigned. There are questions where the units are not independent, too. Treat one user on a social network and their friends change behaviour. The assumption that each person's outcome depends only on their own assignment (the stability-of-treatment-value assumption) breaks, and the standard A/B math quietly gives the wrong answer. And there are one-time events: a policy change, a competitor's pivot, a pandemic. These happen once, to everyone, with no untreated counterfactual. I can still reason about them, but not with the experiment toolkit.
...
There is a piece of vocabulary that quietly runs through everything the book has done, and this is the chapter where it gets named. For each person, there is the outcome they would have had if treated and the outcome they would have had if not treated. I only ever see one of the two. The gap between them, averaged over the population I care about, is what a treatment effect actually is. Randomized experiments work because assignment is independent of those two hidden outcomes, so the averages I can compute line up with the averages I want. Observational data breaks that link, and causal inference is the set of tricks for putting it back together.
