...
Imagine I run a thousand simulated A/B tests on a tech-platform-style problem. Each test has a true effect that varies across tests (some tests have no real effect, some have small effects, some have large effects). I draw those true effects from a bell-shaped population centered at zero, with most effects within a couple of percentage points either way. That population is itself a modeling choice. A platform whose real effects are heavy-tailed, or skewed positive, or mostly zero with a rare big win, would shift the numbers that follow. The shape of the story stays the same, the sharpness of the verdicts does not. For each test, I run both a frequentist decision rule (ship if p < 0.05 and direction is positive) and a Bayesian decision rule (ship if posterior probability of positive effect exceeds 0.95).
...
It is worth noting that these two rules, as teams actually write them down, are not structurally matched. The frequentist rule only asks "is there an effect, in the right direction." The Bayesian rule adds a meaningfulness gate: is the effect bigger than MMU. Part of the gap between them is the lens, part is the gate. When I also run a matched frequentist rule, ship if p < 0.05 and the point estimate is above MMU, the gap in the borderline region shrinks visibly. Most of what looks like a frequentist-versus-Bayesian disagreement is a no-gate-versus-gate disagreement.
...
If I lay the two decisions on a two-by-two table, the picture is even clearer. Both-ship and both-no-ship fill almost the whole table, something like ninety-eight out of a hundred cells. The off-diagonals are thin and asymmetric: the frequentist ships without the Bayesian much more often than the other way round. The Bayesian almost never ships when the frequentist holds back. The cases where the two rules genuinely pull in opposite directions are the small borderline cells worth a careful human look.

One natural follow-up is a joint rule: ship only when both lenses say yes. That rule tracks the more conservative of the two, so it costs a few marginal wins, but it buys a defensible trail. A team that wants to be able to say "we shipped this only because both analytical frames agreed" gets that guarantee for free, at the price of holding back on some of the smaller true effects.
