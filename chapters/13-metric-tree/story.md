...
The frequentist reading of the scatter is the one I just gave: Pearson r around 0.3, and a bootstrap confidence interval across the 200 experiments that sits roughly between 0.15 and 0.41. The Bayesian reading is close to the same number but framed differently. Putting a weakly informative prior on the true correlation and updating on these 200 paired experiments, the posterior over the correlation centres near 0.29, with most of the belief piled up between roughly 0.16 and 0.42. The two intervals line up, which is reassuring, and the interpretations differ in the usual way: the bootstrap interval is a statement about what estimators like this tend to produce in the long run, the posterior is a statement about where I should now place my belief. Either way, the headline number is itself uncertain. A program that reports the short-to-long correlation as if it were a fixed property of the metric pair is overselling what the data knows.
