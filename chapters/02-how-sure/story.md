...
A quick honest admission about that flat starting line. It was my choice, and a different starting line would give a different ending line. If I had walked in believing the coin is probably fair (a hump centered at one half, narrow enough to count as "I expect fair") and then seen sixty heads in a hundred, my belief would barely move. The prior pulls the curve back toward fair. If I had walked in expecting the coin to be rigged low (a hump centered near zero point two), the same sixty heads would drag my belief up, but not above one half. With six hundred heads in a thousand tosses any of these starting lines gets dragged to about the same place, because the data is loud enough to drown out whatever I walked in with. Small samples are where the prior is loud. Large samples are where the data is. ([Try a few starting priors](play/prior_sensitivity.py) and watch the ending curve shift, or not, as N grows.)
