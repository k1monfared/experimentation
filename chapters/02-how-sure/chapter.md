...
- Carried from Chapter 1
  - We saw 6 heads in 10 tosses. The frequentist said "p-value is 0.75, not rare". The Bayesian said "posterior probability of bias is 0.73". Two different answers to almost the same question. Now we make them precise.
...
- Observe
  - The distribution is the binomial. It is centered at 50, with most of the mass between 40 and 60. Counts below 40 or above 60 happen, but only about 5% of the time combined.
  - ![Sampling distribution under H0](images/sampling_distribution_n100.png)
- Frequentist take
  - That picture is a sampling distribution under the null hypothesis H0: p = 0.5. The green bars are the rejection region: counts so unusual that, if H0 were true, we'd see them less than 5% of the time. Anything in green and we say "reject". A two-sided alpha = 0.05 test on N = 100 rejects roughly outside [40, 60].
  ...
  - The thing we will misunderstand all the time: alpha is the rate of *false positives*. If the coin really is fair, alpha is the fraction of experiments that will mislead us into saying "rigged". It is not the probability the coin is rigged given our data. The Bayesian is going to enjoy this distinction.
- Bayesian take
...
- Observe
  ...
  - At N = 100, the curves track each other almost perfectly. Both flag "reject" at roughly the same observed counts.
...
- Probe an edge case
  ...
  - "What if I believe it's probably rigged?" Beta(2, 8) prior. With 6/10 we end up even more confident in bias. With enough N we agree with everyone else.
- Hunch
  - Both lenses can say "I am 95% sure". But "sure of what?" is different: the frequentist is sure that *if H0 were true* we wouldn't see this data; the Bayesian is sure that p sits inside a particular interval. Same number, different statement.
...
- Observe
  - When the truth is exactly fair (p = 0.50), we reject in roughly 5% of experiments at every N. That is the type I error rate, and it equals alpha.
  - When the truth is slightly biased (p = 0.52), we reject in 6% at N=50, 8% at N=200, 18% at N=1000. The signal is too small to detect reliably even at 1000.
  - At p = 0.55: 8%, 25%, 86%. At p = 0.60: 13%, 65%, 99.9%.
