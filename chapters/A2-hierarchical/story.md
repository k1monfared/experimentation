...
It helps to say out loud the three estimators I might reach for once I have these per-segment counts in front of me. The first is no pooling. I treat each segment as its own tiny A/B test, compute its own lift, and print eight numbers. That is what my instinct does when nobody tells it otherwise. A small segment that happened to land a few unlucky tails gets a noisy, wildly wrong number, and I report it with a straight face.

The second is complete pooling. I ignore the segment labels entirely, add the successes and failures across all eight, and compute one global lift. That is a sharp number. It is also the same number for every segment, which means it is the wrong number for any segment whose true effect is not the population average. I have bought precision at the price of erasing the question I wanted to ask.

The third is partial pooling. I say, out loud, that the per-segment effects are themselves draws from a population distribution Normal(mu, tau), and I fit mu, tau, and the per-segment effects jointly. Each segment ends up somewhere between its own no-pooling estimate and the population mean, and the model chooses where based on how much data that segment contributes.

The partial-pooling estimate is not a compromise between the two bad estimators. It is the right estimator for the actual data-generating process, in which segment effects really are draws from a population distribution. The math knows that small segments carry less information and weights them accordingly.

The dial that controls all of this is tau. Tau says how different the segments really are. If tau is tiny, all the segment effects are glued to the population mean and partial pooling looks almost like complete pooling. If tau is large, the segment effects are spread out and partial pooling looks almost like no pooling. In the middle, tau tells me how much to trust each segment's own data against the pull of the population.

The important move is that tau is not a knob I set. Tau is learned. The model has a posterior on tau, and that posterior does the work of deciding how tight the population is. If I simulate segments that are nearly identical, the posterior on tau lands near zero and the model pools hard. If I simulate segments that really do differ, the posterior on tau lands away from zero and the model barely pools at all. I do not pick the pooling strength. The data tells me what it should be.

The one knob I do set is the prior on tau. That is my statement of belief about how different the segments could plausibly be before I see data. With lots of data, the prior on tau is essentially irrelevant. The likelihood drags the posterior wherever it wants. With little data, the prior on tau is the strongest lever in the room. A tight prior (HalfNormal with a small scale) regularizes hard toward "segments are all the same" and collapses everything onto the population mean. A wide prior (HalfNormal with a big scale) keeps the per-segment estimates close to their no-pooling shape but widens every credible interval. Same data, different priors, different pictures. That is the cost of doing hierarchical modeling in a thin-data regime, and the prior has to be defended explicitly.
