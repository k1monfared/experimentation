...
- The recipe
  ...
  - Notice an immediate consequence: the posterior depends on the data only through the likelihood. The stopping rule, the design, and any data we did not see do not enter. This is the likelihood principle, the reason a Bayesian posterior survives optional stopping in Chapter 3.
...
- Why bother with PyMC if the closed form is right there?
  - Because in 99% of real problems the closed form does not exist. PyMC + the same Bayesian recipe extends to: hierarchical models with partial pooling, ratios, transformations, censoring, latent variables, mixtures, anything. We use PyMC where it earns its keep: closed-form Beta when there is one (Chapters 7, 8), hierarchical PyMC when partial pooling matters (Chapters 11, 12), logistic-additive PyMC for attribution (Chapter 17). The closed-form path is not abandoned; it is shown alongside whenever both are tractable.
...
- Compare
  ...
  - Forward pointer: we will not use Bayes factors in the rest of the book. The prior-on-H1 fragility makes them a poor decision tool at scale, and the posterior-probability rule is what we use in Chapters 8 and 18.
