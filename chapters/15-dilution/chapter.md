...
- Observe
  - Intent-to-treat (ITT, all users counted): treatment - control measures around +1.2pp. Why? 12% of treatment users actually got the treatment effect, others got nothing. The headline ITT estimate is roughly 0.12 * 0.10 = 0.012.
  - Per-protocol (visited only): treatment - control among visitors is +10pp, the true effect.
  - ![ITT vs per-protocol](images/itt_vs_per_protocol.png)
- Hunch
  - ITT is a *real* and well-defined estimand: the effect of being in the treatment group, regardless of compliance. Per-protocol is also well-defined but depends on compliance status, which can be correlated with the outcome.

# Loop B: dilution scales with visit rate

- Try
  - Sweep the visit probability from 0.02 to 0.95. For each, measure ITT and per-protocol effects.
- Observe
  - ITT scales linearly with the visit rate. At 100% compliance, ITT = per-protocol.
  - Per-protocol stays near +10pp regardless.
  - ![Dilution sweep](images/dilution_sweep.png)
- Hunch
  - The classic correction is "rescale by 1/compliance" -- the CACE (complier average causal effect). Valid under specific assumptions. Per-protocol naive subsetting is biased when compliance correlates with the outcome.
...
- Observe
  - With per-session revenue lifted by 0.05, the delta method recovers diff = 0.05, se = ~0.05, z = ~1. The naive ratio difference matches but offers no inference.
